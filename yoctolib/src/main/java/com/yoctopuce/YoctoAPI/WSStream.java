...
class WSStream
{
    ...
    WSStream(int type, int channel, int size, ByteBuffer requestBB)
    {
        _channel = channel;
        _type = type;
        _contentLen = size;
    ...
    WSStream(int type, int channel, int size, ByteBuffer requestBB, byte lastByte)
    {
        _channel = channel;
        _type = type;
        _contentLen = size + 1;
    ...
    {
        String content = new String(_data, 1, _contentLen, Charset.forName("ISO-8859-1"));
        String type = "unk";
        switch (_type) {
        ...
        return String.format(Locale.US, "[c=%d t=%s l=%d]:[%s]", _channel, type, _contentLen, content);
    }
